...
export function NavBar() {
  ...
  return (
    <>
      <header className="sticky top-0 z-30 border-b border-sky-200/70 bg-sky-50/80 shadow-sm shadow-sky-500/10 backdrop-blur-xl dark:border-sky-900/50 dark:bg-slate-950/75 dark:shadow-sky-950/20">
        <nav className="mx-auto flex max-w-6xl items-center gap-2 px-6 py-3">
          <Link
            href="/"
            className="mr-5 bg-gradient-to-r from-sky-600 via-blue-600 to-cyan-500 bg-clip-text text-base font-extrabold text-transparent dark:from-sky-300 dark:via-blue-300 dark:to-cyan-300"
          >
            贵州中考英语
          </Link>
          {navItems.map((item) => {
              ...
              <Link
                ...
                className={`rounded-full px-3.5 py-1.5 text-sm font-medium transition-colors ${
                  isActive
                    ...
                    : 'text-slate-600 hover:bg-white/80 hover:text-sky-700 dark:text-sky-100/75 dark:hover:bg-white/10 dark:hover:text-sky-200'
                }`}
              >
                {item.label}
              </Link>
      ...
      <Link
        href="/settings"
        aria-label="模型配置"
        className={`fixed bottom-[10%] left-[1.2rem] z-40 flex h-10 w-10 items-center justify-center rounded-full border shadow-lg backdrop-blur transition-colors ${
          settingsActive
            ? 'border-sky-300 bg-sky-100 text-sky-700 shadow-sky-500/20 dark:border-sky-700 dark:bg-sky-900/40 dark:text-sky-200'
            : 'border-sky-200/80 bg-white/90 text-sky-700 shadow-sky-500/10 hover:bg-sky-50 hover:text-sky-800 dark:border-sky-900/70 dark:bg-slate-900/90 dark:text-sky-200 dark:hover:bg-sky-950/40 dark:hover:text-sky-100'
        }`}
      >
